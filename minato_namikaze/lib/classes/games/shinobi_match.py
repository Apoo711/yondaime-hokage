...
class CharacterSelect(discord.ui.Select["ShinobiMatchCharacterSelection"]):
    ...
    def __fill_options(self) -> None:
        for i in self.characters:
            self.add_option(label=i.name, emoji=i.emoji, value=i.id)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.set_author(
            name=self.view.player.display_name,
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.label.lower() in ["kick", "punch"]:
            ...
            if health < view.overall_health:
                character = (
                    view.character1 if view.turn == view.player1 else view.character2
                )
        ...
        await interaction.message.edit(
            content=f"{view.turn.mention} your stats",
...
class MatchHandlerView(discord.ui.View):
    ...
    def make_embed(
        ...
        author = author or (
            self.player1 if character == self.character1 else self.player2
        )
        embed = Embed()
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.turn and interaction.user in (
            self.player1,
            self.player2,
        ):
            await interaction.response.send_message(
        ...
        if interaction.user != self.turn:
            await interaction.response.send_message(
                "This `Shinobi match` is not for you, please try to be `good spectator`",
                ephemeral=True,
            )
            return False
        return True
    ...
    async def determine_winer(
        ...
        winner_character = (
            self.character1 if winner == self.player1 else self.character2
        )
        looser_character = (
            self.character1 if looser == self.player1 else self.character2
        )

        embed = StarboardEmbed(title=f"{winner_character.name.title()} won the match")
        if force:
            embed.title = (
                f"{winner_character.name.title()} won the match, due the timeout"
            )
